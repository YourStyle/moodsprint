...
- Если задача про ПОКУПКИ и дан список продуктов — создай шаги "Купить [категория]: [товары]"
- Если задача про путешествие/прогулку — предложи конкретные места
- Если задача про обучение — предложи ресурсы, темы
- Если в описании перечислены конкретные пункты — используй ИХ, не выдумывай свои
- Используй свои знания чтобы ОБОГАТИТЬ задачу, НО НЕ РАЗДУВАЙ простые задачи!

ПРИМЕРЫ ПРАВИЛЬНОГО РАЗБИЕНИЯ:
- "Купить продукты" + "молоко, хлеб, яйца" → "Купить: молоко, хлеб, яйца" (5 мин)
- "Сходить в магазин" + 10 товаров → 2-3 шага по категориям
