...
from app.models import (
    ...
    Task, Subtask, FocusSession, MoodCheck
)
...
def get_user_stats():
    ...
    total_subtasks = db.session.query(func.count(Subtask.id)).join(Task).filter(
        Task.user_id == user_id,
        Subtask.status == SubtaskStatus.COMPLETED.value
    ).scalar()

    # Total focus minutes
    total_focus_minutes = db.session.query(
    ...
    today_subtasks = db.session.query(func.count(Subtask.id)).join(Task).filter(
        Task.user_id == user_id,
        Subtask.status == SubtaskStatus.COMPLETED.value,
        Subtask.completed_at >= today_start
    ).scalar()

    today_focus_minutes = db.session.query(
        func.coalesce(func.sum(FocusSession.actual_duration_minutes), 0)
    ).filter(
        FocusSession.user_id == user_id,
        FocusSession.status == FocusSessionStatus.COMPLETED.value,
    ...
    today_mood_checks = MoodCheck.query.filter(
        ...
        MoodCheck.created_at >= today_start
    ).count()
...
def get_daily_goals():
    ...
    subtasks_completed = db.session.query(func.count(Subtask.id)).join(Task).filter(
        Task.user_id == user_id,
        Subtask.status == SubtaskStatus.COMPLETED.value,
        Subtask.completed_at >= today_start
    ).scalar() or 0
