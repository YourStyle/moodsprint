...
def get_active_session():
    ...
    if not session:
        session = FocusSession.query.filter_by(
            user_id=user_id,
            status=FocusSessionStatus.PAUSED.value
        ).first()
...
def complete_focus_session():
    ...
    xp_earned = XPCalculator.focus_session_completed(session.actual_duration_minutes or 0)
...
def get_focus_history():
    ...
    query = FocusSession.query.filter(
        ...
        FocusSession.status.in_([
            ...
            FocusSessionStatus.CANCELLED.value
        ])
